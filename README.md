...
## Feed 로딩 시 정렬/페이징 전략
- 피드 조회는 `@Query`에서 `Post`와 `Follow`를 조인해 팔로우 관계를 한 번에 필터링합니다.
- 페이징은 커서 기반(`cursorId`)이며, 서버에서 해당 포스트의 `createdAt`을 조회해 `createdAt`과 `id`를 함께 비교합니다.
- 정렬은 `posts.created_at desc, posts.id desc`로 최신 포스트가 먼저 오도록 구성합니다.
- 정렬 기준과 커서 기준을 동일하게 유지해 중복/누락을 방지합니다.
- 기본 페이지 크기는 `size` 파라미터로 제어합니다.
- `Slice`를 사용해 전체 건수 `count` 쿼리를 생략하고, 다음 페이지 존재 여부만 판단합니다.
